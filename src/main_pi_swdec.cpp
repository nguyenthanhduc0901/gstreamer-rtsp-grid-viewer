// Linux/GTK SWDEC variant: force software H.265 decode (avdec_h265) for high-res streams
// - Window: GTK with 2x2 grid using gtksink widgets
// - Pipelines:
//   * cam1, cam2, cam4: rtspsrc (latency=0, drop-on-lateness) -> rtph265depay -> h265parse -> avdec_h265 -> queue(leaky) -> videoscale -> caps(SUB_WxSUB_H) -> videoconvert -> gtksink
//   * cam3: rtspt + decodebin (but we demote v4l2slh265dec rank to avoid HW decoder; decodebin should pick avdec_h265)
// - Low latency tactics: latency=0, drop-on-lateness=true, sink sync=false async=false, leaky queue
...
static void demote_hw_h265_decoder() {
    GstRegistry* reg = gst_registry_get();
    if (!reg) return;
    const char* features[] = { "v4l2slh265dec", "v4l2h265dec", "v4l2hevcdec" };
    for (const char* name : features) {
        GstPluginFeature* feat = gst_registry_find_feature(reg, name, GST_TYPE_ELEMENT_FACTORY);
        if (feat) {
            gst_plugin_feature_set_rank(feat, GST_RANK_NONE);
            g_printerr("Demoted feature rank: %s\n", name);
...
int main(int argc, char** argv) {
    ...
    // Ensure decodebin won't pick HW H.265 decoder for high-res
    demote_hw_h265_decoder();

    GtkWidget* window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(window), "GStreamer 2x2 (GTK, SWDEC)");
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Only cam3 uses decodebin per your gst-launch tests
        ...
        g_object_set(G_OBJECT(sp->sink), "sync", FALSE, NULL);
        g_object_set(G_OBJECT(sp->sink), "async", FALSE, NULL);
        g_object_set(G_OBJECT(sp->sink), "force-aspect-ratio", TRUE, NULL);

        g_object_set(G_OBJECT(sp->queue), "leaky", 2, "max-size-buffers", 1, "max-size-bytes", 0, "max-size-time", 0, NULL);
